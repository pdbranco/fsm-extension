...
function initializeRefreshTokenStrategy(shellSdk, auth) {
  ...
  shellSdk.on(SHELL_EVENTS.Version1.REQUIRE_AUTHENTICATION, (event) => {
    sessionStorage.setItem('tokenServiceContract', event.access_token);
  ...
  sessionStorage.setItem('tokenServiceContract', auth.access_token);
...
function getServiceContract(cloudHost, account, company, activity_id) {
  ...
  const headers = {
    ...
    'Authorization': `bearer ${sessionStorage.getItem('tokenServiceContract')}`,
  };
